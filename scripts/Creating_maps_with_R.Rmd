...
the first task in drawing a map is to get a data frame with the right information in it, and in the right order. First we load R’s maps package, which provides us with some pre-drawn map data
...
```
 ...
 lets can map other variables . Let’s try a continuous measure, such as the percentage of the vote received by Donald Trump.
```{r}
p <- us_states_elec %>% 
  ggplot(aes(long,lat,group = group,fill = pct_trump))+
  geom_polygon(color = "gray90", size = 0.1)+
  coord_map(projection = "albers", lat0 = 39, lat1 = 45)
